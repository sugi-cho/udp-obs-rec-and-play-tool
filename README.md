...
- RECタブ
  - OBS接続（WebSocket v5）
  - 受信したUDPをローカルIP/別ポートへ常時即時中継
  - `REC START` で OBS録画開始 + 中継中UDPのJSONL記録開始
  - `REC STOP` で JSONL記録停止 + OBS録画停止
- PLAYタブ
...
- ステータス表示
  - REC: `OBS / FWD / REC / Packets / Log` を1行表示 + 受信UDP直近4件（累積連番）
...
- `REC` の中継先ポートは、受信用 `UDP Listen Port` とは別の値を指定してください。
- 中継はアプリ起動中は `REC` 非実行時でも継続します。`REC` はログ保存のON/OFFです。
